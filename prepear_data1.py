    ...
    if match_trlit:
        if ind == 4:
            if str(match_trlit.group(0))[:3] == "Vs." or str(match_trlit.group(0))[2:4] == "y.":
                ...
                text = text[:pos_start_trlit] + "1" + text[end:]
            else:
                pos_start_trlit = match_trlit.start()
        else:
            pos_start_trlit = match_trlit.start()
...
def extract_tabletVII(text: str, start_pos: int, pattern: str, ind: int):
    ...
    text_translate_prev = ""
    global Pattern_search_trlit, Pattern_search_trlit_re, Pattern_search_translate, Pattern_search_translate_re, Pattern_search_translate_end, Pattern_search_translate_end_re
    global patterns_kultepe_VII_s, patterns_kultepe_VII_per_s, patterns_kultepe_VII_per_e, number_text, prov_prev_transl
    if start_pos < 0 or start_pos >= len(text):
        return (text_translate, ""), flag_vyp, start_pos
    end_pos = len(text)
    if start_pos == 0:
...
def align_and_mark_sentences(translit_text: str, translation_sentences: list, marker="<sent>") -> str:
    ...
    # def gale_church_align(translit_text, translation_sentences, marker="<sent>"):

    # translit_tokens = translit_text.split()
    # trans_lengths = [len(s.split()) for s in translation_sentences]
    #
    # T = len(translit_tokens)
    # S = len(trans_lengths)
    #
    # if S == 0:
    #     return translit_text + " " + marker
    #
    # avg_ratio = T / sum(trans_lengths)
    # variance = 6.8  # стандартная эмпирическая константа Gale-Church
    #
    # # DP таблица
    # dp = [[math.inf] * (T + 1) for _ in range(S + 1)]
    # back = [[None] * (T + 1) for _ in range(S + 1)]
    #
    # dp[0][0] = 0
    #
    ...
    #                 continue
    #
    #             len_s = sum(trans_lengths[s:ns])
    #             len_t = nt - t
    #
    #             expected = len_s * avg_ratio
    #
    #             z = (len_t - expected) / math.sqrt(variance)
    #
    #             cost = z * z
    #
    #             val = dp[s][t] + cost
    ...
    #     marked_tokens.extend(translit_tokens[start:end])
    #     marked_tokens.append(marker)
    #
    # return " ".join(marked_tokens)
...
def process_text_and_build_csv_rows(text: str):
    ...
    all_patterns = [pattern1, pattern2, pattern3, pattern4, pattern5, pattern6, pattern7, pattern8, pattern9]
    ...
    extract_function_1 = [extract_quoted_substring, extract_ankara, extract_ankara_next, extract_letter_space_digit_colon_space, extract_numbs_and_diapasons, extract_numbs_and_diapasons, extract_salim_assur, extract_sebahat, extract_tabletVII]
    ...
    extract_function_2 = [extract_parenthesized_substring, extract_after_ankara, extract_after_ankara_next, extract_after_letter_space_digit_colon_space, after_extract_numbs_and_diapasones, after_extract_numbs_and_diapasones, after_extract_salim_assur, after_extract_sebahat, after_extract_tabletVII]
    str_txt = [""] * len_arr
    str_txt_1 = [""] * len_arr
